...
class MainWindowAppFactory(object):
    ...
    def getComponents(self):
        components = {
            ...
            'ViewBehaviors' : ['View'],
            'PluginManager' : [],
            'Grid': ['View', 'ObjectModel', 'PluginManager'],
            'PointCloud': ['View', 'ObjectModel', 'PluginManager'],
            'MainWindow' : ['View', 'ObjectModel'],
            # 'AdjustePluginManagerdClippingRange' : ['View'],
    ...
    def initGrid(self, fields):
        grid = director.devel.plugins.core.Grid(None, fields.view, parent='scene')
        return grid.init()

    def initPointCloud(self, fields):
        pcloud = director.devel.plugins.geometry.XYZPointCloudRenderer(None, fields.view, None, 'point_cloud')
        return pcloud.init()


    def initPluginManager(self, fields):
        pm = PluginManager()
        pm.list_all_plugins()
    ...
    #       'plugin',
    #       '/code/myapp/components/core/.py').load_module(); dviz.sim_editor.IDMAgentEditingSubPanel = ddviz.IDMAgentEditingSubPanel
    #
    #
    #
    #     from core import GridPlugin
    #     grid = GridPlugin(None, fields.view, parent='scene')
    #     return grid.init()
